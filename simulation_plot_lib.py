...
def calculate_metrics(overwrite=False):
    ...
    for color in metrics["colors"]:
        ...
        color_metrics["win_rates"] = np.mean(player_data["games_won"])

        color_metrics["average_tokens_captured"] = np.mean(
            player_data["tokens_captured"]
        )
        color_metrics["average_tokens_lost"] = np.mean(player_data["tokens_beaten"])
        color_metrics["average_squares_moved"] = np.mean(
            player_data["total_squares_moved"]
        )

        turns_until_win = [x for x in player_data["turns_until_win"] if x is not False]
        color_metrics["average_turns_until_win"] = (
            np.mean(turns_until_win) if turns_until_win else 0
        )
        ...
        for turns_taken, captured_tokens, tokens_beaten, won in zip(
            player_data["turns_taken"],
            player_data["tokens_captured"],
            player_data["tokens_beaten"],
...
def player_metric_pie(
    metric_name: str, title: str, red=True, green=True, blue=True, orange=True
):
    calculate_metrics()
    ...
    for color in metrics["colors"]:
        if enabled[color]:
            ...
            if isinstance(color_metrics[metric_name], list):
                y_data = [
                    value
                    for value in color_metrics[metric_name]
    ...
    plt.pie(
        ...
        labels=[metrics[color]["strategy"] for color in metrics["colors"]],
        autopct="%1.0f%%",
    )
    plt.title(title)
    ...
    for color_index, color in enumerate(metrics["colors"]):
        if enabled[color]:
            ...
            for metrics_index, metric in enumerate(metric_names):
                special_color = (
                    colors[metrics_index] if metrics_index < len(colors) else color
                )
                special_label = (
                    labels[metrics_index] if metrics_index < len(labels) else None
                )
                y_data = [
                    value
                    for value in color_metrics[metric]
                    if not isinstance(value, bool)
                ]
...
def player_metric_bars(
    ...
    for color_index, color in enumerate(metrics["colors"]):
        if enabled[color]:
            ...
            for metrics_index, metric in enumerate(metric_names):
                special_color = (
                    colors[metrics_index] if metrics_index < len(colors) else None
                )
                ...
                plt.bar(
                    color_index + bar_width * metrics_index,
                    y_data,
                    color=special_color,
                    edgecolor=color,
                    width=bar_width,
                    label=labels[metrics_index],
                )

    plt.xticks(
        [r + bar_width / 2 for r in range(len(metrics["colors"]))],
        [metrics[color]["strategy"] for color in metrics["colors"]],
    )
    plt.xlabel("Strategies")
    plt.ylabel(y_label)
    plt.title(title)
...
def player_data_histogram(
    ...
    max_value = max(
        [
            value
            for sublist in [data["players"][color][data_name] for color in included]
    ...
    if overlapping:
        for color in metrics["colors"]:
            if enabled[color]:
                ...
                y_data = [
                    value
                    for value in data["players"][color][data_name]
                    if not isinstance(value, bool)
                ]
                plt.hist(
                    y_data,
        ...
        plt.hist(
            y_datas,
            bins,
            label=[metrics[color]["strategy"] for color in included],
            color=[color for color in included],
            density=normalize,
        )
    plt.xlabel(x_label)
    plt.ylabel(y_label)
...
def player_data_gauss_fit(
    ...
    for color in metrics["colors"]:
        if enabled[color]:
            ...
            plt.plot(
                x, norm.pdf(x, mean, std), label=metrics[color]["strategy"], color=color
            )
...
def player_data_scatter(
    data_name1: str, data_name2: str, x_label: str, y_label: str, title: str
):
    calculate_metrics()
    ...
    for index, color in enumerate(metrics["colors"]):
        x = [
            value
            for value in data["players"][color][data_name1]
            if not isinstance(value, bool)
        ]
        y = [
            value
            for value in data["players"][color][data_name2]
            if not isinstance(value, bool)
        ]
        c = Counter(zip(x, y))
...
player_metric_line(
    metric_name="games_won_by_tokens_captured",
...
player_metric_line(
    metric_name="games_lost_by_tokens_beaten",
    x_label="Tokens Beaten",
    y_label="Games Lost",
    title="Games Lost by Tokens Beaten",
)

player_metric_line(
    metric_name="games_won_by_turns_taken",
    x_label="Turns Taken",
    y_label="Games Won",
...
player_metric_lines(
    metric_names=["games_won_by_turns_taken", "games_lost_by_turns_taken"],
    x_label="Turns Taken",
...
player_metric_bar(
    metric_name="win_rates",
    y_label="Percentage of Games Won",
    title="Strategy Winrates",
)

player_metric_pie(metric_name="win_rates", title="Strategy Winrates")
...
player_data_histogram(
    data_name="total_squares_moved",
...
player_data_scatter(
    data_name1="total_squares_moved",
